...
import communication_no_stream as communication
...
class StageRuntime:
    def __init__(self, model, distributed_backend, fp16, loss_scale,
        ...
        self.forward_stats = runtime_utilities.RuntimeStats(forward=True, rank=rank, epoch=0)
        self.backward_stats = runtime_utilities.RuntimeStats(forward=False, rank=rank, epoch=0)
    ...
    def initialize(self, model, inputs_module_destinations,
        ...
        if module_to_stage_map is None:
            ...
            self.num_previous_warmup_minibatches = 0
            ...
            if stage_to_depth_map is not None:
                self.num_warmup_minibatches = stage_to_depth_map[
                    str(self.stage)]
                if self.stage > 0:
                    self.num_previous_warmup_minibatches = stage_to_depth_map[
                    str(self.stage - 1)]
            else:
                self.num_warmup_minibatches = self.num_ranks - 1
                self.num_previous_warmup_minibatches = self.num_ranks - 1
                for i in range(self.stage):
                    self.num_warmup_minibatches -= len(
                        stage_to_rank_map[i])
                    if i != self.stage - 1:
                        self.num_previous_warmup_minibatches -= len(
                            stage_to_rank_map[i])
                self.num_warmup_minibatches = self.num_warmup_minibatches // \
                    self.num_ranks_in_stage
                self.num_previous_warmup_minibatches = self.num_previous_warmup_minibatches // \
        ...
        if self.comm_handler is not None:
            self.comm_handler.initialize(
                ...
                self.ranks_in_previous_stage,
                self.ranks_in_next_stage,
                self.num_warmup_minibatches,
                self.num_previous_warmup_minibatches)
    ...
    def train(self, num_iterations):
        ...
        if self.comm_handler is not None:
            ...
            self.comm_handler.start_helper_threads(
                num_iterations, forward_only=False, epoch=self.epoch)
    ...
    def run_forward(self, recompute_step=False):
        ...
        self.forward_stats.stats['receive_tensors'] += (t_2-t_1) * 1000.00
        self.forward_stats.stats['compute_time'] += (t_3-t_2) * 1000.00
        self.forward_stats.stats['send_tensors'] += (t_4-t_3) * 1000.00
        self.forward_stats.stats['total_time'] += (t_4-t_1) * 1000.00
    ...
    def run_backward(self):
        ...
        self.backward_stats.stats['receive_tensors'] += (t_2-t_1) * 1000.00
        self.backward_stats.stats['compute_time'] += (t_3-t_2) * 1000.00
        self.backward_stats.stats['send_tensors'] += (t_4-t_3) * 1000.00
        self.backward_stats.stats['total_time'] += (t_4-t_1) * 1000.00
