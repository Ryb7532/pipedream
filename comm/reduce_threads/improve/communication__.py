...
class CommunicationHandler(object):
    ...
    def start_helper_threads(self, num_iterations, forward_only, epoch):
        ...
        """
        if forward_only:
            self.set_counter(self.num_ranks_in_previous_stage + \
                             self.num_ranks_in_next_stage)
        else:
            self.set_counter((self.num_ranks_in_previous_stage + \
                              self.num_ranks_in_next_stage)*2)
    ...
    def recv_helper_thread_args(self, index, backward, stream, schedule, epoch):
        tensor_names = []
        targets = []
        recv_queues = {}
        tensor_shapes = {}
    ...
    def send_helper_thread_args(self, index, backward, stream, schedule, epoch):
        tensor_names = []
        targets = []
        send_queues = {}
        tags = {}
...
def send_helper_thread(send_queues, counter, local_rank, tensor_names,
    ...
    # This method is to be executed from a helper daemon thread.
    with torch.cuda.stream(stream):
        send_list = targets + tensor_names
        q = queue.Queue()
        for i in schedule:
