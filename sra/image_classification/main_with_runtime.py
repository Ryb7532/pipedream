...
def main():
    ...
    if args.arch == 'inception_v3':
        if args.synthetic_data:
            train_dataset = SyntheticDataset((3, 299, 299), 122880)
        else:
        ...
        if args.synthetic_data:
            train_dataset = SyntheticDataset((3, 224, 224), 106496)
        else:
            traindir = os.path.join(args.data_dir, 'train')
            train_dataset = datasets.ImageFolder(
                traindir,
    ...
    if args.synthetic_data:
        val_dataset = SyntheticDataset((3, 224, 224), 122880)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.forward_only:
            ...
            prec1 = 0 #validate(val_loader, r, epoch)
...
def train(train_loader, r, optimizer, epoch):
    ...
    for i in range(n - num_warmup_minibatches):
        ...
        #     stream.synchronize()
        #     _end = time.time()
        #     batch_time.update(_end - end)
        #     end = _end
        #     epoch_time = (end - epoch_start_time) #/ 3600.00
        ...
        #               'Epoch time [s]: {epoch_time:.3f} ({full_epoch_time:.3f})\t'
        #               'Memory: {memory:.3f} ({cached_memory:.3f})\t'
        #               'Loss: {loss.val:.4f} ({loss.avg:.4f})\t'
        #               'Prec@1: {top1.val:.3f} ({top1.avg:.3f})\t'
        #               'Prec@5: {top5.val:.3f} ({top5.avg:.3f})'.format(
        #                epoch, i, n, args.rank, batch_time=batch_time,
        ...
        # else:
        #     if i % args.print_freq == 0:
        #         print('Epoch: [{0}][{1}/{2}]\tRank: {3}\tMemory: {memory:.3f} ({cached_memory:.3f})'.format(
        #                epoch, i, n, args.rank, memory=(float(torch.cuda.memory_allocated()) / 10**9),
        #                cached_memory=(float(torch.cuda.memory_cached()) / 10**9)))
        #         import sys; sys.stdout.flush()
    ...
    stream.synchronize()
    wait_time = time.time()
    print("%.6lf : Rank %d : wait : epoch %d" % (wait_time, args.rank, epoch))
    r.wait()

    r.forward_stats.print_stats()
    r.forward_stats.reset_stats()
    r.backward_stats.print_stats()
    r.backward_stats.reset_stats()
    ...
    print("Wait_time (Rank %d ): %.6f seconds" % (args.rank, end-wait_time))
