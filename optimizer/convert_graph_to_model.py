...
if __name__ == '__main__':
    ...
    all_template_args.append({
        ...
        "stage_to_depth_map": str({"0": 0}).replace("'", "\"")
    })
    all_template_args.append({
        ...
        "stage_to_depth_map": str({str(i): (num_modules-2-i) * 3 for i in range(num_modules-1)}).replace("'", "\"")
    })
    if args.stage_to_num_ranks_map is not None:
        stage_to_rank_map = {}
        stage_to_depth_map = {}
        ...
        # for i in reversed(range(1, num_modules-1)):
        #     stage_to_depth_map[str(i)] = warmup_in_stage
        #     warmup_in_stage = ((warmup_in_stage+1) * stage_to_num_ranks_map[i][1] - 2) // stage_to_num_ranks_map[i-1][1] + 2
        # stage_to_depth_map[str(0)] = warmup_in_stage
        d = 0
        for i in reversed(range(num_modules-1)):
            stage_to_depth_map[str(i)] = d
            if i > 0:
                if stage_to_num_ranks_map[i][1] > stage_to_num_ranks_map[i-1][1]:
                    d += math.ceil((stage_to_num_ranks_map[i][1]) / stage_to_num_ranks_map[i-1][1]) * 2 #+ math.ceil((stage_to_num_ranks_map[i][1]+1) / stage_to_num_ranks_map[i-1][1])
                else:
                    d += 3
        stage_to_depth_map = str(stage_to_depth_map).replace("'", "\"")
        all_template_args.append({
            "module_to_stage_map": list(range(num_modules-1)) + [num_modules-2],
            "stage_to_rank_map": stage_to_rank_map,
            "stage_to_depth_map": stage_to_depth_map
        })
