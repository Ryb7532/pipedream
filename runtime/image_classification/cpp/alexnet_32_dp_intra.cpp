...
/*
  string cmd = "nvprof --profile-child-processes -o alexnet_32_dp_intra_";
  cmd += stm0.str();
...
*/
  ...
  cmd += " --node_rank ";
  cmd += stm0.str();
  cmd += " --nproc_per_node 4 main_with_runtime.py -s --distributed_backend gloo -m models.alexnet.gpus=32 --epochs 6 -b 8 --config_path models/alexnet/gpus=32/dp_conf.json --no_input_pipelining --num_ranks_in_server 4 --master_addr ";
  cmd += (string)argv[1];
