...
#        world_group = dist.new_group(ranks=range(self.world_size))
        ...
        dist.all_gather(gathered_connection_list_sizes,
                        connection_list_size)#, group=world_group)
        ...
        dist.all_gather(aggregated_connection_list,
                        connection_list_tensor)#, group=world_group)
#        print(aggregated_connection_list)
...
#                    print("create group (in rank %d): %d, %d" % (self.rank, min_rank, max_rank),flush=True)
#                    dist.barrier()
    ...
    def start_helper_thread(self, args_func, func, args_func_args):
        """
...
#                                         received_tensor_shape))
        ...
        # Receive tensor.
#        tensor = torch.zeros(receive_tensor_shape, dtype=dtype).cuda()
