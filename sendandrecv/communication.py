...
class CommunicationHandler(object):
    ...
    def schedule_for_helper_threads(self, num_iterations):
        ...
        if self.num_ranks_in_next_stage > 0:
            ...
            recv = send
            warmup = max(1,self.num_warmup)
            for i in range(warmup):
                forward_schedules[send].append(
                    (True, self.rank_in_stage + i * self.num_ranks_in_stage))
                send = (send + self.num_ranks_in_stage) % \
                    self.num_ranks_in_next_stage
            for i in range(num_iterations - warmup):
                forward_schedules[recv].append(
                    (False, self.rank_in_stage + i * self.num_ranks_in_stage))
                recv = (recv + self.num_ranks_in_stage) % \
                    self.num_ranks_in_next_stage
                forward_schedules[send].append(
                    (True, self.rank_in_stage + (i+warmup) * self.num_ranks_in_stage))
                send = (send + self.num_ranks_in_stage) % \
                    self.num_ranks_in_next_stage
            for i in range(num_iterations - warmup, num_iterations):
                forward_schedules[recv].append(
                    (False, self.rank_in_stage + i * self.num_ranks_in_stage))
                recv = (recv + self.num_ranks_in_stage) % \
                    self.num_ranks_in_next_stage
        ...
        if self.num_ranks_in_previous_stage > 0:
            ...
            recv_from = [i % self.num_ranks_in_stage for i in range(self.num_ranks_in_previous_stage)]
            send_from = [i % self.num_ranks_in_stage for i in range(self.num_ranks_in_previous_stage)]
            warmup = max(1,self.num_previous_warmup)
            for i in range(warmup):
                for j in range(self.num_ranks_in_previous_stage):
                    if recv_from[j] == self.rank_in_stage:
                        backward_schedules[j].append(
                            (False, j + i * self.num_ranks_in_previous_stage))
                    recv_from[j] = (recv_from[j] + self.num_ranks_in_previous_stage) % \
                        self.num_ranks_in_stage
            for i in range(num_iterations-warmup):
                for j in range(self.num_ranks_in_previous_stage):
                    if send_from[j] == self.rank_in_stage:
                        backward_schedules[j].append(
                            (True, j + i * self.num_ranks_in_previous_stage))
                    send_from[j] = (send_from[j] + self.num_ranks_in_previous_stage) % \
                        self.num_ranks_in_stage
                for j in range(self.num_ranks_in_previous_stage):
                    if recv_from[j] == self.rank_in_stage:
                        backward_schedules[j].append(
                            (False, j + (i+warmup) * self.num_ranks_in_previous_stage))
                    recv_from[j] = (recv_from[j] + self.num_ranks_in_previous_stage) % \
                        self.num_ranks_in_stage
            for i in range(num_iterations - warmup, num_iterations):
                for j in range(self.num_ranks_in_previous_stage):
    ...
    def start_helper_threads(self, num_iterations, forward_only, epoch):
        ...
        """
        self.set_counter(self.num_ranks_in_previous_stage + self.num_ranks_in_next_stage)
        ...
        for i in range(self.num_ranks_in_previous_stage):
            self.start_helper_thread_(
                self.send_and_recv_helper_thread_args,
                send_and_recv_helper_thread,
                [i, True, stream,
    ...
    def send_and_recv_helper_thread_args(self, index, comm_across_previous, stream, schedule, epoch):
        dst_rank = None
        tensor_names = []
        targets = []
        recv_queues = {}
...
def send_and_recv_helper_thread(recv_queues, send_queues, counter, local_rank,
                        tensor_names, targets, src_rank, dst_rank, tags,
                        tensor_shapes, dtypes, sub_process_group, stream,
                        schedule, epoch):
    if not schedule:
        counter.decrement()
        return
